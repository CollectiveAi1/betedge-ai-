...
const later = new Date(today.getTime() + 3 * 60 * 60 * 1000).toISOString();
const evening = new Date(today.getTime() + 7 * 60 * 60 * 1000).toISOString();
...
export function getMockGames(sport?: SportKey): MockGame[] {
  ...
  return MOCK_GAMES.filter((g: MockGame) => g.sport === sport);
}
